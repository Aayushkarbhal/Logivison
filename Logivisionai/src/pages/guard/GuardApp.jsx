...
const ScanTab = () => {
  ...
  const triggerVibrate = () => {
    if (
      localStorage.getItem("setting_vibrate") !== "false" &&
      navigator.vibrate
    ) {
  ...
  useEffect(() => {
    ...
    const requestWakeLock = async () => {
      try {
        if (
          localStorage.getItem("setting_awake") === "true" &&
        ...
        try {
          ...
          if (result.success) {
            setPlateData({
              ...
              details:
                result.data.vehicleNo === "ERROR" ? result.data.details : "",
            });
            triggerVibrate();
            setScanAnim("DONE");
            setPlateDone(true);
  ...
  const toggleManualMode = () => {
    if (!isManualMode) {
      setManualData({
        ...
        items: extractedChallanData?.goodsDescription || "",
        weight: extractedChallanData?.totalWeight || "",
        value: extractedChallanData?.totalValue || "",
  ...
  const handleSubmitEntry = async () => {
    ...
    try {
      ...
      const challan = normalize(
        extractedChallanData?.vehicleNo || manualData.vehicleNo,
      );

      const isPlateMatch = isManualMode || plate === challan;
      const declared = isManualMode
        ? manualData.capacity
        : extractedChallanData?.declaredLoad || "EMPTY";
      ...
      if (!isPlateMatch && !isEscalated && !isManualMode) {
        ...
        showToast(
          "⚠️ Vehicle No Mismatch — Manual override enabled",
          "warning",
        );
      }
      ...
      const entryData = isManualMode
        ? {
            ...
            dockAssigned:
              manualData.to && manualData.to.length <= 6 ? manualData.to : "", // Auto-map if it looks like a dock ID
        ...
        : {
            ...
            goodsDescription: extractedChallanData?.goodsDescription,
            totalWeight: extractedChallanData?.totalWeight || "0",
            totalValue: extractedChallanData?.totalValue || "0",
            declaredLoad: declared,
            dockAssigned:
              extractedChallanData?.destination &&
              extractedChallanData.destination.length <= 6
                ? extractedChallanData.destination
                : "",
            visualLoad: loadCheck,
            status: "PENDING", // Always PENDING to force Manager Vibe Check
            scan_method: "ocr",
            imageUrl: challanImage,
            vehicleImageUrl: loadPhoto,
          };
      ...
      if (data.success) {
        ...
        showToast(
          isLoadMatch
            ? "Entry Verified & Logged"
            : "Mismatch Logged — Manager Notified",
          "success",
        );
    ...
    return (
      <div className="absolute inset-0 bg-[#080C14] z-50 flex flex-col items-center justify-center p-6 pb-24 h-screen">
        <div className="bg-[#0D1421] border border-[#0DD9B0] w-full max-w-sm rounded-[32px] p-8 text-center shadow-2xl shadow-[#0DD9B0]/10">
          ...
          <h2 className="text-2xl font-bold text-[#0DD9B0] mb-2 tracking-tight">
            ENTRY CLEARED
          </h2>
          <div className="bg-[#080C14] rounded-2xl border border-[#1E2D45] p-5 text-left space-y-4 mb-8 mt-4">
            <div className="flex justify-between items-center text-xs">
              <span className="text-[#6B7FA8] font-black uppercase tracking-wider">
                Vehicle
              </span>
          ...
          <button
            onClick={resetFlow}
            className="w-full bg-gradient-to-r from-[#0DD9B0] to-[#0BA68E] text-black font-black py-4 rounded-2xl shadow-lg shadow-[#0DD9B0]/20 active:scale-95 transition-transform"
          >
            SCAN NEXT TRUCK
          </button>
        </div>
    ...
    return (
      <div className="absolute inset-0 bg-[#080C14] z-50 flex flex-col items-center justify-center p-6 pb-24 h-screen">
        <div className="bg-[#0D1421] border border-[#F43F5E] w-full max-w-sm rounded-[32px] p-8 text-center shadow-2xl shadow-[#F43F5E]/10">
          ...
          <h2 className="text-xl font-bold text-white mb-8 bg-[#F43F5E] py-2 rounded-xl">
            MISMATCH LOGGED
          </h2>
          <div className="w-full bg-[#080C14] border border-[#1E2D45] p-6 rounded-2xl mb-8 space-y-4 text-left">
            <div className="flex justify-between text-xs font-black">
              <span className="text-[#6B7FA8] uppercase tracking-wider">
                TRUCK NO:
              </span>
          ...
          <div className="space-y-3">
            <button
              onClick={handleFlag}
              className="w-full bg-[#F43F5E] text-white font-black py-4 rounded-xl shadow-lg shadow-[#F43F5E]/20"
            >
              NOTIFY MANAGER
            </button>
            <button
              onClick={resetFlow}
              className="w-full border border-[#1E2D45] text-[#6B7FA8] font-black py-4 rounded-xl"
            >
              RE-SCAN
            </button>
          </div>
    ...
    <div className="flex flex-col h-full bg-[#080C14]">
      ...
      <div className="sticky top-0 z-[100] glass-header px-5 py-4 border-b border-[#1E2D45] flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] font-black text-[#6B7FA8] uppercase tracking-[0.1em] mb-0.5">
              Gate 03 · Panvel
            </div>
        ...
        <div className="flex flex-col items-end">
          <div className="text-[9px] font-black text-[#6B7FA8] uppercase tracking-wider mb-0.5">
            SHIFT PROGRESS
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono text-[#0DD9B0] font-black">
              {feed.length} Scans
            </span>
            <div
              className={`w-2 h-2 rounded-full ${isOffline ? "bg-[#F43F5E]" : "bg-[#0DD9B0]"} animate-pulse shadow-[0_0_8px] shadow-current`}
            ></div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-6 pt-6">
        <div className="px-5 space-y-8">
            ...
            <div className="bg-industrial-gradient border border-[#1E2D45] rounded-[28px] overflow-hidden shadow-2xl relative p-2 shadow-amber-glow">
              <div className="relative aspect-[4/3] rounded-[20px] overflow-hidden bg-black flex items-center justify-center border border-[#FFFFFF08]">
                ...
                <input
                  ...
                  accept="image/*"
                  capture="environment"
                  onChange={handleLoadPhotoSelect}
                  className="hidden"
                />
                ...
                <div className="w-full h-full relative">
                  {plateImage && scanTarget === "PLATE" ? (
                    ...
                    <img
                      src={challanImage}
                      className="w-full h-full object-cover opacity-60 backdrop-sepia-[0.2]"
                      alt="Challan"
                    />
                  ) : loadPhoto ? (
                    <img
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="text-[10px] text-[#6B7FA8] font-black uppercase tracking-[0.2em]">
                  ...
                  {scanAnim === "PARSING" && (
                    <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] flex items-center justify-center z-[60]">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <div className="text-[#F59E0B] text-[10px] font-black tracking-widest uppercase animate-pulse font-mono">
                          NEURAL_OCR_ACTIVE
                        </div>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className={`absolute top-4 right-4 p-2.5 rounded-xl border-2 backdrop-blur-xl transition-all z-[80] ${
                      plateDone
                        ? "bg-[#0DD9B0]/20 border-[#0DD9B0] text-[#0DD9B0]"
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Camera
                        size={14}
                        className={plateDone ? "animate-pulse" : ""}
                      />
                      <span className="text-[10px] font-black tracking-widest uppercase">
                        {plateDone ? "PLATE LOCKED" : "PLATE SCAN"}
                      </span>
                    </div>
                  </button>
                  ...
                  {mismatchCount > 0 && (
                    <div className="absolute bottom-4 left-4 flex gap-1.5 p-2 bg-black/40 backdrop-blur-md rounded-lg border border-white/10 z-[80]">
                      {[...Array(4)].map((_, i) => (
                        <div
                          key={i}
                          className={`w-1.5 h-1.5 rounded-full ${i < mismatchCount ? "bg-[#F43F5E] shadow-[0_0_5px_#F43F5E]" : "bg-white/10"}`}
                        ></div>
                      ))}
                    </div>
                  )}
                ...
                {showChallanPreview && challanImage && (
                  <div className="absolute inset-0 bg-[#080C14]/95 backdrop-blur-xl flex flex-col items-center justify-center p-8 z-[120] animate-fade-in">
                    ...
                    <div className="text-white font-black text-sm uppercase tracking-widest text-center mb-1">
                      Verify Quality
                    </div>
                    <div className="text-[#6B7FA8] text-[10px] uppercase font-bold tracking-wider mb-8 text-center px-4">
                      Ensure optical clarity for neural extraction
                    </div>
                    <div className="flex flex-col gap-3 w-full">
                      <button
                        onClick={processChallanOCR}
                        className="w-full bg-[#F59E0B] text-black font-black py-4 rounded-xl shadow-lg shadow-[#F59E0B]/20 uppercase text-xs tracking-widest"
                      >
                        PROCEED TO AI
                      </button>
                      <button
                        ...
                        className="w-full border border-[#1E2D45] text-[#6B7FA8] font-black py-3.5 rounded-xl text-[10px] uppercase"
                      >
                        Discard & Retake
                      </button>
                    </div>
          ...
          {challanDone && extractedChallanData && !isManualMode && (
            <div className="mx-5 bg-[#111827] border border-[#F59E0B]/30 rounded-[28px] p-6 shadow-2xl animate-fade-in">
              ...
              <div className="space-y-4">
                {[
                  ...
                  {
                    label: "GOODS DESC",
                    val: extractedChallanData.goodsDescription,
                    req: true,
                  },
                  { label: "WEIGHT", val: extractedChallanData.totalWeight },
                  {
                    label: "TOTAL VALUE",
                    val: extractedChallanData.totalValue,
                    req: true,
                  },
                ].map((item, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center bg-[#080C14] px-4 py-3 rounded-xl border border-[#1E2D45]"
                  >
                    <span className="text-[9px] font-black text-[#6B7FA8] uppercase tracking-wider">
                      {item.label}
                    </span>
                    {item.val && item.val !== "null" && item.val !== "N/A" ? (
                      ...
                      <span
                        className={`text-[10px] font-black uppercase ${item.req ? "text-[#F43F5E] animate-pulse" : "text-[#3D4F6B]"}`}
                      >
          ...
          <div className="flex flex-col items-center gap-5">
            {mismatchCount < 4 ? (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-base font-black tracking-widest uppercase leading-none">
                    Scan Challan
                  </div>
                </div>
              </button>
            ...
            <div className="flex items-center gap-6">
              <button
                onClick={handleGalleryChallan}
                className="text-[10px] text-[#6B7FA8] font-bold uppercase tracking-widest border-b border-[#1E2D45] pb-0.5"
              >
                GALLERY UPLOAD
              </button>
              <button
                onClick={toggleManualMode}
                className={`text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full border transition-all ${isManualMode ? "bg-[#0DD9B0]/20 border-[#0DD9B0] text-[#0DD9B0]" : "bg-[#1E2D45] border-[#1E2D45] text-[#6B7FA8]"}`}
              >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-[9px] text-[#F43F5E] font-black uppercase tracking-widest">
                Mandatory Sensor
              </span>
            </div>
              ...
              {[
                {
                  ...
                  label: "FULL",
                  icon: Truck,
                  activeClass: "border-[#0DD9B0] bg-[#0DD9B0]/10 text-white",
                },
                {
                  id: "HALF",
                  label: "HALF",
                  icon: Truck,
                  activeClass: "border-[#F59E0B] bg-[#F59E0B]/10 text-white",
                },
                {
                  id: "EMPTY",
                  label: "EMPTY",
                  icon: Truck,
                  activeClass: "border-white bg-white/10 text-white",
                },
              ].map((opt) => (
                <button
                  key={opt.id}
                ...
                >
                  <opt.icon
                    size={28}
                    className={
                      loadCheck === opt.id ? "opacity-100" : "opacity-40"
                    }
                  />
            ...
            {loadPhoto && (
              <div className="bg-[#111827] border border-[#1E2D45] p-3 rounded-2xl flex items-center justify-between animate-fade-in group">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[10px] text-[#0DD9B0] font-black uppercase tracking-wider mb-0.5">
                      SENSORY DATA LOGGED ✓
                    </div>
                    <div className="text-[9px] text-[#6B7FA8] font-bold uppercase">
                      Visual proof anchored to block
                    </div>
                  </div>
                </div>
                <button
                  onClick={() => setLoadPhoto(null)}
                  className="p-2 text-[#3D4F6B] hover:text-[#F43F5E] transition-colors"
                >
                  <XSquare size={20} />
                </button>
              </div>
            ...
            <button
              ...
              className={`w-full h-14 rounded-2xl border-2 border-dashed flex items-center justify-center gap-3 transition-all ${loadPhoto ? "border-[#0DD9B0] bg-[#0DD9B0]/5 text-[#0DD9B0]" : "border-[#1E2D45] text-[#6B7FA8] hover:border-[#F59E0B]/30"}`}
            >
              <Camera size={20} />
              <span className="text-[10px] font-black uppercase tracking-widest">
                {loadPhoto
                  ? "CHANGE HUD PHOTO"
                  : t("SCAN_LOAD") || "CAPTURE VISUAL HUD"}
              </span>
            </button>
          </div>
          ...
          {isManualMode && (
            <div className="bg-[#111827] rounded-[28px] p-6 border border-[#1E2D45] space-y-6 shadow-2xl animate-fade-in">
              <div className="flex items-center justify-between border-b border-[#1E2D45] pb-4">
                ...
                <div className="bg-[#F59E0B]/10 text-[#F59E0B] text-[8px] px-2 py-0.5 rounded-full font-black border border-[#F59E0B]/20 uppercase">
                  Overwrite Mode
                </div>
              </div>
              <div className="space-y-4">
                {[
                  {
                    label: t("VEHICLE_NO"),
                    key: "vehicleNo",
                    placeholder: "MH XX 0000",
                  },
                  {
                    label: "Consignment Entity (FROM)",
                    key: "from",
                    placeholder: "Vendor / Transporter",
                    required: true,
                  },
                  {
                    label: "Logistics Dock (TO)",
                    key: "to",
                    placeholder: "D-07, B-12",
                    required: true,
                  },
                  {
                    label: "Goods Description",
                    key: "items",
                    placeholder: "Electronics, Sensors, etc.",
                    required: true,
                  },
                  {
                    label: "Declared Mass (Weight)",
                    key: "weight",
                    placeholder: "Mass in Tons",
                  },
                  {
                    label: "Total Value (₹)",
                    key: "value",
                  ...
                  <div key={f.key}>
                    <div className="flex justify-between items-center mb-1.5 ml-1">
                      <label className="text-[9px] font-black text-[#6B7FA8] uppercase tracking-[0.2em]">
                        {f.label}
                      </label>
                      {!manualData[f.key] && f.required && (
                        <span className="text-[8px] font-black text-[#F43F5E] uppercase animate-pulse">
                          Required
                        </span>
                      )}
                    </div>
...
const HistoryTab = () => {
  ...
  const displayedFeed = feed.filter(
    (f) =>
      activeTab === "ALL" ||
      (activeTab === "MISMATCH" && f.status === "MISMATCH") ||
      activeTab === "TODAY",
  );
    ...
    <div className="flex flex-col h-full bg-[#080C14]">
        ...
        <div className="flex gap-2 mt-6 overflow-x-auto hide-scrollbar">
          {["ALL", "TODAY", "MISMATCH"].map((tab) => (
            <button
              ...
              className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all tracking-widest uppercase ${tab === activeTab ? "bg-[#F59E0B] text-black shadow-lg shadow-[#F59E0B]/20" : "bg-[#1A2235] text-[#6B7FA8] border border-[#1E2D45]"}`}
            >
              {tab}
            </button>
          ))}
      ...
      <div className="flex-1 overflow-y-auto px-6 space-y-4 pb-24">
        {displayedFeed.length === 0 ? (
          <div className="text-center text-[#6B7FA8] text-xs py-12 uppercase tracking-widest font-bold">
            No Records Found
          </div>
        ) : (
          displayedFeed.map((item) => (
            <div
              key={item.id}
              className="bg-[#1A2235] border border-[#1E2D45] p-4 rounded-2xl flex items-center justify-between shadow-lg"
            >
              <div>
                <div className="font-mono text-white font-black text-base">
                  {item.plate || "—"}
                </div>
                <div className="text-[10px] text-[#6B7FA8] font-black uppercase mt-1">
                  {item.vendor || "Unknown Source"}
                </div>
              </div>
    ...
    <div className="flex flex-col h-full bg-[#080C14]">
      <div className="px-6 pt-10 pb-6 shrink-0">
        <div className="flex justify-between items-center">
          ...
          {mismatches.length > 0 && (
            <span className="bg-[#F43F5E] text-white text-[10px] font-black px-2 py-0.5 rounded-full">
              {mismatches.length} Active
            </span>
          )}
        </div>
        ...
        {mismatches.length === 0 ? (
          ...
          mismatches.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className="bg-[#1A2235] border border-l-4 border-l-[#F43F5E] border-[#1E2D45] rounded-xl overflow-hidden shadow-xl"
              >
                <div className="p-5 space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="font-mono text-lg font-black text-[#F59E0B]">
                      {alert.vehicleNo || alert.plate}
                    </span>
                    <span className="text-[9px] text-[#6B7FA8] font-bold uppercase">
                      {alert.timeAgo || "SEC_RELEVANT"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-[10px]">
                    <div>
                      <span className="text-[#6B7FA8] uppercase font-black block mb-0.5">
                        Declared
                      </span>
                    ...
                    <div>
                      ...
                      <span className="text-[#F43F5E] font-black">
                        {alert.visualLoad}
                      </span>
                    </div>
                  </div>
                  <div
                    className={`p-2 rounded text-[10px] font-black uppercase text-center ${alert.status === "awaiting" ? "bg-[#F59E0B]/10 text-[#F59E0B]" : "bg-[#F43F5E]/10 text-[#F43F5E]"}`}
                  >
                    {alert.status === "awaiting"
                  ...
                  <div className="flex gap-2 pt-2">
                    <button
                      ...
                      className="flex-1 border border-[#1E2D45] text-white text-[10px] font-black py-2.5 rounded-lg uppercase tracking-wider"
                    >
                      {isEx ? "Hide Details" : "Full Specs"}
                    </button>
                    <button
                      onClick={() => showToast("Escalated to HQ", "info")}
                      className="flex-1 bg-[#F43F5E] text-white text-[10px] font-black py-2.5 rounded-lg uppercase tracking-wider shadow-lg shadow-[#F43F5E]/20"
                    >
                      Escalate
                    </button>
                  </div>
    ...
    <div className="flex flex-col h-full bg-[#080C14]">
      <div className="flex flex-col items-center pt-16 pb-8 bg-industrial-gradient border-b border-[#1E2D45]">
        ...
        <h2 className="text-2xl font-black text-white uppercase tracking-widest">
          {user?.name || "Vijay Yadav"}
        </h2>
        <div className="mt-2 px-4 py-1.5 rounded-full bg-[#F59E0B]/10 border border-[#F59E0B]/30 text-[#F59E0B] text-[10px] font-black uppercase tracking-widest">
          {user?.role === "GUARD"
            ? "Master Security Unit"
            : "Logistics Intelligence"}
        </div>
        <div className="mt-4 text-[10px] text-[#6B7FA8] font-black uppercase tracking-widest opacity-60">
          Gate 03 · Industrial Logistic Hub
        </div>
      </div>
      ...
      <div className="p-6 space-y-6 flex-1 overflow-y-auto pb-32 hide-scrollbar">
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div
              key={i}
              className="bg-[#111827] border border-[#1E2D45] rounded-2xl p-4"
            >
              <div className="text-[9px] font-black text-[#6B7FA8] uppercase tracking-widest mb-1">
                {s.label}
              </div>
              <div
                className={`text-xl font-black ${s.color} font-mono tracking-tight`}
              >
                {s.val}
              </div>
            </div>
        ...
        <div className="bg-[#111827] border border-[#1E2D45] rounded-2xl p-5 space-y-4">
          <div className="text-[10px] font-black text-[#F59E0B] uppercase tracking-widest border-b border-[#1E2D45] pb-3 mb-2">
            {t("LANGUAGE")}
          </div>
          ...
          {[
            {
              ...
              state: localStorage.getItem("setting_vibrate") !== "false",
              icon: Vibrate,
              toggle: () => {
                const current =
                  localStorage.getItem("setting_vibrate") !== "false";
                localStorage.setItem("setting_vibrate", (!current).toString());
                // Force re-render trick for simple toggle
            ...
            },
            {
              ...
              state: localStorage.getItem("setting_audio") !== "false",
              icon: Volume2,
              toggle: () => {
                const current =
                  localStorage.getItem("setting_audio") !== "false";
                localStorage.setItem("setting_audio", (!current).toString());
                document.dispatchEvent(new Event("storage"));
              },
            },
            {
              ...
              state: localStorage.getItem("setting_awake") === "true",
              icon: Clock,
              toggle: () => {
                const current =
                  localStorage.getItem("setting_awake") === "true";
                localStorage.setItem("setting_awake", (!current).toString());
                document.dispatchEvent(new Event("storage"));
              },
            },
          ].map((pref, i) => (
            <div key={i} className="flex items-center justify-between">
              ...
              <button
                onClick={pref.toggle}
                className={`w-10 h-5 rounded-full p-1 transition-all ${pref.state ? "bg-[#0DD9B0]" : "bg-[#1E2D45]"}`}
              >
                <div
                  className={`w-3 h-3 rounded-full bg-[#080C14] transition-all ${pref.state ? "translate-x-5" : "translate-x-0"}`}
                ></div>
              </button>
            </div>
          ))}
        ...
        <button
          ...
          className="w-full h-14 border-2 border-[#F43F5E] text-[#F43F5E] font-black text-sm uppercase tracking-widest rounded-2xl flex items-center justify-center gap-3 hover:bg-[#F43F5E]/10 active:scale-95 transition-all"
        >
          <LogOut size={20} />
          {t("LOGOUT")}
        </button>
