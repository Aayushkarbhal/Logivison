...
const FreightTab = ({ isLive, toggleLive }) => {
  ...
  // Zero-Overbooking — real-time stock levels keyed by inventoryId
  ...
  useEffect(() => {
    const fetchMarketData = async () => {
      try {
        ...
        // 4. Fetch Global Marketplace Inventory (Cargo Pool)
        const invRes = await fetch(`/api/inventory`, { headers });
        const invData = await invRes.json();
        if (invData.success) {
          const invMap = {};
          invData.data.inventoryItems.forEach(item => {
            // Calculate available quantity
            const avail = item.currentQty - item.reservedQty - (item.allocatedQty || 0);
            invMap[item._id] = {
              ...item,
              available: avail
            };
  ...
  useEffect(() => {
    ...
    // 📡 Zero-Overbooking: react to live stock changes
    const handleInventoryUpdate = (data) => {
      setLiveStock(prev => ({ ...prev, [data.inventoryId]: data }));

      // If manager hides the item — trigger fade-out animation
      if (!data.marketplaceVisible) {
        setFadingOut(prev => ({ ...prev, [data.inventoryId]: true }));
        showToast(`📦 "${data.name}" removed from marketplace by manager`, 'warning');
      } else {
        setFadingOut(prev => ({ ...prev, [data.inventoryId]: false }));
      }

      // If stock just hit 0 — alert driver
      if (data.available <= 0) {
        showToast(`⚠️ ${data.name}: OUT OF STOCK — booking button disabled`, 'error');
      }
    };
    ...
    return () => {
      socket.off('freight:available', handleNewFreight);
  ...
  return (
    <div className="flex flex-col h-full bg-[#080C14] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
        ...
        {/* 📦 WAREHOUSE CARGO POOL (New Section) */}
        <div className="space-y-4">
          <div className="flex items-center justify-between ml-2">
            <h3 className="text-[#38BDF8] text-[10px] font-bold uppercase tracking-[0.2em]"> Warehouse Cargo Pool</h3>
            <span className="text-[9px] text-gray-500 font-bold uppercase">Ready for Shipment</span>
          </div>
          ...
          >
            {Object.values(liveStock).filter(item => item.marketplaceVisible !== false).map(item => (
              <div key={item._id} className="bg-[#0D1421] border border-[#1E2D45] rounded-xl p-4 flex flex-col justify-between">
                <div>
                  <div className="text-[9px] text-[#38BDF8] font-bold uppercase tracking-wider mb-1">{item.category}</div>
                  <h4 className="text-white font-bold text-xs line-clamp-2 mb-2">{item.name}</h4>
                </div>
                
                <div className="space-y-2">
                  <div className="flex flex-col gap-1">
                    <span className="text-[8px] text-gray-500 font-bold uppercase tracking-tighter">Available Stock</span>
                    <div className="flex items-baseline gap-1">
                      <span className={`text-sm font-mono-data font-black ${item.available <= 0 ? 'text-[#F43F5E]' : 'text-[#0DD9B0]'}`}>
                        {item.available}
                      </span>
                      <span className="text-[9px] text-gray-400 font-bold">{item.unit || 'pcs'}</span>
                    </div>
                  </div>
                  
                  <div className="w-full h-1 bg-[#1E2D45] rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all ${item.available <= 0 ? 'bg-[#F43F5E]' : 'bg-[#0DD9B0]'}`} 
                      style={{ width: `${Math.min(100, (item.available / (item.currentQty || 1)) * 100)}%` }}
                    />
                  </div>
                  
                  {item.available > 0 ? (
                    <div className="text-[9px] text-[#0DD9B0] font-black uppercase tracking-tighter flex items-center gap-1">
                      <Zap size={10} fill="#0DD9B0" /> BOOKABLE
                    </div>
                  ) : (
                    <div className="text-[9px] text-[#F43F5E] font-black uppercase tracking-tighter">OUT OF STOCK</div>
                  )}
                </div>
              </div>
            ))}
            {Object.values(liveStock).filter(item => item.marketplaceVisible !== false).length === 0 && (
              <div className="w-full py-8 text-center bg-[#0D1421]/30 rounded-xl border border-dashed border-[#1E2D45]">
                <p className="text-[10px] text-gray-600 font-bold uppercase">No cargo in pool</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {incomingJobs.length === 0 ? (
              ...
              incomingJobs.map(job => {
                // Resolve live stock for this job's linked inventory
                const invId = job.inventoryId?._id || job.inventoryId;
                const stockInfo = invId ? liveStock[invId] : null;
                const availableQty = stockInfo ? stockInfo.available : (job.inventoryId?.currentQty ?? Infinity);
                const isHidden = stockInfo ? !stockInfo.marketplaceVisible : false;
                const isOutOfStock = availableQty <= 0;
                const isFading = isHidden || fadingOut[invId];

                if (isHidden) return null; // remove from view if manager toggled off

                return (
                  <div
                    key={job._id}
                    className={`bg-[#1A2235] rounded-2xl p-5 border border-[#1E2D45] shadow-2xl relative overflow-hidden group transition-all duration-700 ${
                      isFading ? 'opacity-20 scale-95 pointer-events-none' : 'opacity-100 scale-100'
                    }`}
                  >
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        ...
                        {/* Live stock badge */}
                        {invId && (
                          <div className={`mt-1 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${
                            isOutOfStock ? 'bg-[#F43F5E]/10 text-[#F43F5E]' : 'bg-[#0DD9B0]/10 text-[#0DD9B0]'
                          }`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${ isOutOfStock ? 'bg-[#F43F5E]' : 'bg-[#0DD9B0] animate-pulse'}`}></span>
                            {isOutOfStock ? 'OUT OF STOCK' : `${availableQty} ${job.inventoryId?.unit || 'units'} Available`}
                          </div>
                        )}
                      </div>
                      <div className="text-right">
                    ...
                    {/* Dynamic CTA — Zero-Overbooking logic */}
                    {isOutOfStock ? (
                      <div className="space-y-3">
                        <div className="w-full bg-[#1E2D45] text-[#6B7FA8] font-black py-4 rounded-xl text-xs uppercase tracking-widest text-center cursor-not-allowed border border-[#3D4F6B]">
                          🚧 OUT OF STOCK
                        </div>
                        <button
                          onClick={() => setNotifyMe(prev => ({ ...prev, [job._id]: !prev[job._id] }))}
                          className={`w-full py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${
                            notifyMe[job._id]
                              ? 'bg-[#F59E0B]/10 border-[#F59E0B] text-[#F59E0B]'
                              : 'bg-transparent border-[#3D4F6B] text-[#6B7FA8]'
                          }`}
                        >
                          {notifyMe[job._id] ? '🔔 Notify Me: ON' : '🔔 Notify Me When Available'}
                        </button>
                      </div>
                    ) : (
                      <button
                        onClick={() => handleAcceptJob(job._id)}
                        className="w-full bg-[#0DD9B0] hover:bg-[#0BB898] text-black font-black py-4 rounded-xl transition-all shadow-[0_4px_20px_rgba(13,217,176,0.25)] text-xs uppercase tracking-widest active:scale-95"
                      >
                        ✓ BOOK FREIGHT — {invId ? `${availableQty} ${job.inventoryId?.unit || 'units'} avail.` : 'Accept Now'}
                      </button>
                    )}
                  </div>
                );
