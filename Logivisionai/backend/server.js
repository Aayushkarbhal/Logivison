...
io.on("connection", (socket) => {
  console.log(
    `User connected: ${socket.user.name} (${socket.user.email}) - Role: ${socket.user.role}`,
  );
  ...
  socket.on("disconnect", () => {
    console.log(
      `User disconnected: ${socket.user.name} (${socket.user.email})`,
    );
  });
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const todayCount = await Challan.countDocuments({
        ...matchQuery,
        createdAt: { $gte: startOfToday },
      });
      const weekCount = await Challan.countDocuments({
        ...matchQuery,
        createdAt: { $gte: startOfWeek },
      });
      const monthCount = await Challan.countDocuments({
        ...matchQuery,
        createdAt: { $gte: startOfMonth },
      });
      const mismatchCount = await Challan.countDocuments({
        ...matchQuery,
        status: "MISMATCH",
        createdAt: { $gte: startOfToday },
      });
      ...
      const verifiedCount = await Challan.countDocuments({
        ...matchQuery,
        status: "VERIFIED",
        createdAt: { $gte: startOfToday },
      });
      const approvalRate =
        todayCount > 0 ? ((verifiedCount / todayCount) * 100).toFixed(1) : 0;
...
app.get("/api", (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      challans: {
        ...
        "PATCH /api/challans/:id/status":
          "Update challan status (Authority/Admin only)",
      ...
      docks: {
        ...
        "PATCH /api/docks/:id/assign":
          "Assign truck to dock (Warehouse Manager/Admin only)",
        "PATCH /api/docks/:id/release":
          "Release dock (Warehouse Manager/Admin only)",
        "PATCH /api/docks/:id/schedule":
          "Schedule dock (Warehouse Manager/Admin only)",
        "GET /api/docks/stats/warehouse": "Get dock statistics",
        "PATCH /api/docks/:id/maintenance": "Update dock maintenance status",
      },
      inventory: {
        "GET /api/inventory":
          "Get all inventory items (Warehouse Manager/Admin only)",
        "GET /api/inventory/:sku": "Get single inventory item",
        "POST /api/inventory":
          "Create new inventory item (Warehouse Manager/Admin only)",
        "PATCH /api/inventory/:id":
          "Update inventory item (Warehouse Manager/Admin only)",
      ...
      freight: {
        ...
        "POST /api/freight/bookings":
          "Create new freight booking (Warehouse Manager only)",
        "PATCH /api/freight/bookings/:id/accept":
          "Accept booking (Driver only)",
        "GET /api/freight/bookings":
          "Get bookings filtered by driver or warehouse",
        "GET /api/freight/bookings/:id": "Get single booking",
        "PATCH /api/freight/bookings/:id/location":
          "Update booking location (Driver only)",
        "PATCH /api/freight/bookings/:id/complete":
          "Complete booking (Driver only)",
        "PATCH /api/freight/bookings/:id/cancel":
          "Cancel booking (Warehouse Manager/Admin only)",
    ...
    socketEvents: {
      ...
      "dock:maintenance:updated":
        "Emitted when dock maintenance status changes",
