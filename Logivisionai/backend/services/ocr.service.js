...
const extractJson = (text) => {
  try {
    ...
    if (start === -1 || end === -1) {
      log(`[JSON_FIX] No braces found in: ${text.substring(0, 50)}...`);
      return null;
    }
    ...
    log(`[JSON_FIX] Parse failed: ${e.message}`);
    return null;
  }
};
...
const processChallanImage = async (imageSource) => {
  ...
  log(
    `[Gemini-HardFix] Processing. MIME: ${mimeType}, Size: ${base64Data.length}`,
  );
  ...
  1. Challan ID (Label: "Challan ID:")
  2. Truck number (Label: "Truck number:")
  3. From (Label: "FROM:")
  4. To (Label: "TO:")
  5. Goods Description (Label: "Goods Description:")
  6. Weight (Label: "Weight:")
  7. Total value (Label: "Total value:")
  8. Capacity (Label: "Capacity:")
  ...
  try {
    ...
    if (!parsed || !parsed.truck_number) {
      log(
        `[Gemini-HardFix] Standard JSON failed or truck number missing. Running Regex Safety Net.`,
      );
      const truckMatch = text.match(
        /[A-Z]{2}\s?[0-9]{2}\s?[A-Z]{0,2}\s?[0-9]{4}/i,
      );
      if (!parsed) parsed = {};
